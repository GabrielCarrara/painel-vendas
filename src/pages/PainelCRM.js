...
import {
  ...
  FaEdit, FaTrash, FaSave, FaTimes, FaExclamationTriangle, FaSpinner, FaEye,
} from "react-icons/fa";
    ...
    >
      ...
      >
        ...
        <div className="px-5 py-4 border-t border-gray-700 bg-gray-900/40 flex flex-wrap justify-end gap-3">
          <button type="button" onClick={onCancelar} className="px-5 py-2.5 rounded-lg font-semibold bg-gray-600 hover:bg-gray-500 text-white transition-colors">
            Cancelar
          </button>
          <button type="button" onClick={onConfirmar} className="px-5 py-2.5 rounded-lg font-semibold bg-red-600 hover:bg-red-500 text-white transition-colors">
            Excluir
          </button>
        </div>
      </div>
...
export default function PainelCRMAprimorado({
  ...
  return (
    <div className="animate-fade-in text-white">
      ...
      {isDiretor && (
        <div className="mb-6 flex flex-col sm:flex-row sm:items-end gap-4 bg-gray-800/50 rounded-xl p-5 border border-gray-700/50">
          <div className="flex-1">
            <label className="block text-sm font-medium text-gray-400 mb-2">Visualizar CRM de</label>
            <select
      ...
      {!somenteLeitura && (
        <div className="bg-gray-800/50 rounded-xl shadow-2xl p-6 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs text-gray-400">Data de retorno <span className="text-gray-500">(opcional)</span></label>
            ...
            <textarea placeholder="Observação" value={novoLead.observacao} onChange={(e) => setNovoLead({ ...novoLead, observacao: e.target.value })} className="bg-gray-700 p-3 rounded-lg border border-gray-600 focus:ring-2 focus:ring-indigo-500 md:col-span-2 lg:col-span-3" rows="2" />
          </div>
      ...
      <div className="bg-gray-800/50 rounded-xl shadow-2xl p-6">
        <h3 className="text-xl font-bold mb-4 flex items-center gap-2">
          <FaUsers /> Lista de Leads
          {isDiretor && usuarioLeadsId !== usuarioLogadoId && (
            <span className="text-sm font-normal text-gray-400">— {nomeUsuarioFiltro}</span>
          )}
        </h3>
          ...
          <table className="min-w-full text-sm text-left">
            <thead className="border-b border-gray-700">
              <tr className="text-gray-400 uppercase">
                ...
                {!somenteLeitura && <th className="px-4 py-3 text-center">Ações</th>}
              </tr>
            </thead>
            <tbody>
                  ...
                  <tr key={lead.id} className="border-b border-gray-700/50 hover:bg-gray-700/50 transition-colors">
                    {editandoId === lead.id ? (
                      <>
                        ...
                        <td className="p-2"><textarea value={leadEditado.observacao} onChange={(e) => setLeadEditado({ ...leadEditado, observacao: e.target.value })} className="bg-gray-600 p-2 rounded w-full" rows="2" /></td>
                        <td className="p-2">
                        ...
                        {!somenteLeitura && (
                          <td className="px-4 py-4 text-center">
                            <div className="flex gap-3 justify-center">
                              ...
                              <button type="button" onClick={() => abrirConfirmacaoExcluir(lead)} className="p-2 text-red-500 hover:text-red-400" title="Excluir lead">
                                <FaTrash size={18} />
                              </button>
                            </div>
                          </td>
